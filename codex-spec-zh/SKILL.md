---
name: codex-spec-zh
description: 使用 spec-plan-progress-acceptance 工作流管理 Codex 中长研发任务，支持任务新建、切换、归档、软删除与恢复；当用户提到长任务规划、跨前后端联调、里程碑验收、多项目复用、中文任务管理时使用。
---
...
执行长任务时遵循以下固定流程：
1. 先写 `spec.md`，再写 `plan.md`。
2. 一次只执行一个 step。
3. 每步执行后必须更新 `progress.md`。
4. 阶段完成后更新 `acceptance.md`。
5. 任务完结后归档，不直接删除历史。
...
如果用户只说“执行下一步”，先读取 `ACTIVE_TASK` 与当前任务目录，再基于 `plan.md` 选择下一个未完成步骤。
...
4. 未验证通过不得标记完成。
5. 遇到冲突先写入 `progress.md` 的 BLOCKED，再请求决策。
...
3. `references/progress.template.md`
4. `references/acceptance.template.md`
5. `references/checklist-vue-spring.md`

优先使用中文输出；命令与文件路径保持英文。
