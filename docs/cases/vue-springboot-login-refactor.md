...
如果把这个案例直接交给 Codex，建议让它按 `任务说明 -> 代码库地图 -> 执行计划 -> 分步执行 -> 每步验证 -> 验收` 的顺序推进，不要一次性把所有代码都改完。
...
建议先从这些位置开始找：
1. 前端登录页、路由守卫、请求拦截器。
2. 后端登录 Controller、鉴权 Filter、统一异常处理。
3. 与 token、session、权限相关的配置文件。
...
3. 直接相关文件。
4. 潜在影响文件。
5. 搜索关键词（如 `login`、`token`、`auth`、`jwt`）。
...
1. 填写 `验收记录.md`。
2. 确认 `代码库地图.md` 和 `验证记录.md` 已按需补齐。
3. 确认 `status` 里显示的当前任务目录和中文文档路径无误。
4. 通过后执行：
